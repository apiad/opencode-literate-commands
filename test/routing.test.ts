...
describe("resolveNextStep", () => {
  ...
  it("should evaluate conditional map", () => {
    ...
    expect(resolveNextStep(conditions, steps, { role: "guest" })).toBe(0) // falls to _
  })
  ...
  it("should use first match in conditional map", () => {
    ...
    expect(resolveNextStep(conditions, steps, { role: "admin" })).toBe(1) // first match
    expect(resolveNextStep(conditions, steps, { role: "other" })).toBe(2) // second match
  })
  ...
  it("should fall back to _ when no condition matches", () => {
    ...
    expect(resolveNextStep(conditions, steps, { role: "guest" })).toBeNull() // no match, no fallback
  })
  ...
  it("should handle special characters in condition", () => {
    const steps = createSteps()
    expect(resolveNextStep({ "name.includes('John')": "admin-panel" }, steps, { name: "John Doe" })).toBe(1)
  })
  ...
  it("should handle empty metadata with _ fallback", () => {
    ...
    expect(resolveNextStep(conditions, steps, {})).toBe(2) // falls to _
  })
})
